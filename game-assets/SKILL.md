...
5. **Animation:** animate only after the character or prop design is stable. Before running, follow the high-quality animation workflow in [Animation and video](references/animation-and-video.md); it defines prompt preparation, automatic prompt enhancement, size-based mode routing, and mandatory transparent motion space.
...
4. **Animation:** finalize the still asset before animation, then follow the prompt, mode-routing, and motion-canvas checks in [Animation and video](references/animation-and-video.md). Use `animate-run` for most game animation. For an ordinary but complex action, prepare intermediate poses and use `keyframes-run` before considering video. Use `video-run` only as a fallback for motion that frame animation cannot represent or when higher video resolution is required.
