...
difficulty: "advanced"
tags: ["bac", "access-control", "privilege-escalation", "spring-boot", "web", "ctf"]
language: "zh-CN"
last_updated: "2026-07-04"
related_articles: ["ctf-website/14-idor/01-idor-enumeration", "ctf-website/17-api-attacks/01-api-discovery-leak", "ctf-website/15-mass-assignment/01-mass-assignment", "ctf-website/12-payment/payment-logic", "ctf-website/24-database/02-sqli-advanced"]
---
...
BAC 要按“角色 × HTTP 方法 × API 版本 × 业务状态”打矩阵，尤其盯订单和支付。很多题的入口不是 `/admin`，而是普通用户能直接触发 `confirm/refund/ship/export/reconcile` 这类高权限动作，副作用落在数据库状态和支付流水上。

| 维度 | 重点样本 | 业务 oracle | 下一跳 |
|---|---|---|---|
| 角色 | anonymous/user/vendor/staff/admin | 响应字段、菜单、按钮、任务 ID | IDOR 对象替换 |
| 方法 | GET/POST/PUT/PATCH/DELETE/OPTIONS | `Allow`、状态码、动作副作用 | 方法覆写、参数污染 |
| 流程状态 | cart/pending/paid/refunded/shipped/closed | `status`, `paid_at`, `refund_id` | 支付状态机 |
| API 版本 | v0/v1/v2/internal/admin | 同资源不同鉴权结果 | API discovery |
| Parser | JSON/form/XML/multipart | 同字段不同绑定路径 | Mass Assignment |
| 数据库动作 | export/search/bulk/update/delete | 记录数、排序、报错、任务文件 | SQLi、批量对象越权 |

矩阵执行器：

```python
# bac_role_method_matrix.py
import csv
import json
import requests

PAYMENT_ACTIONS = ("pay", "paid", "confirm", "refund", "charge", "capture", "ship", "invoice")

def short_body(resp):
    text = resp.text.replace("\n", " ")
    return text[:300]

def classify_side_effect(resp):
    text = resp.text.lower()
    if any(k in text for k in ("paid_at", "refund_id", "transaction_id", "invoice_id")):
        return "payment-state-leak"
    if any(k in text for k in ("sql", "syntax", "database", "where", "constraint")):
    ...
    if resp.status_code in (404, 405):
        return "route-method-signal"
    return "business-error"

def run_matrix(base_url, endpoints, role_tokens, out_csv="exports/bac_role_method_matrix.csv"):
    rows = []
    for role, token in role_tokens.items():
        ...
        for ep in endpoints:
            for method in ("GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"):
                ...
                r = s.request(method, url, json={"status": "paid", "paid_at": "2099-01-01"}, timeout=10, allow_redirects=False)
                rows.append({
                    "role": role,
                    "method": method,
                    "endpoint": ep,
                    "status": r.status_code,
                    "oracle": classify_side_effect(r),
                    "payment_like": any(x in ep.lower() for x in PAYMENT_ACTIONS),
                    "body": short_body(r),
                })
...
def diff_state(before_json, after_json):
...
1. 端点名出现 `order/checkout/payment/refund/invoice/coupon/export/bulk/search`，优先进入矩阵。
2. `OPTIONS` 暴露 `PATCH/DELETE`，马上尝试方法覆写和版本切换。
3. `202 accepted` 不等于结束，继续查任务详情、导出文件、队列状态和订单回读接口。
4. 响应出现 SQL 报错、排序字段、过滤 DSL、批量更新计数，跳数据库文档。
5. 响应出现 `paid_at/refund_id/transaction_id/currency/amount` 差异，跳支付逻辑文档。

Evidence 固定输出：

- `bac_role_method_matrix.csv`：角色、方法、端点、状态码、业务 oracle。
- `state_transition_diff.jsonl`：每次动作前后的订单/支付/用户状态 diff。
- `bac_payment_candidates.json`：从 API discovery 提取的订单、支付、退款、导出端点。
- `bac_database_oracle.jsonl`：SQL 报错、排序差异、批量影响行数。
