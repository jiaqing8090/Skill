...
邮件题不要只看 SPF 是否存在，要同时看 `MAIL FROM`、`Header.From`、DKIM `d=` 三者是否对齐。

| MAIL FROM | Header.From | DKIM d= | SPF | DKIM | DMARC 结果 | 可利用信号 |
|---|---|---|---|---|---|---|
| `bounce@target.com` | `admin@target.com` | 无 | pass | fail | 取决于 SPF alignment | SPF pass 但 DKIM 缺失 |
| `bounce@mail.target.com` | `admin@target.com` | 无 | pass | fail | relaxed 可能 pass | 子域名宽松对齐 |
| `bounce@evil.test` | `admin@target.com` | `target.com` | fail | pass | pass | DKIM 私钥或签名链可用 |
| `bounce@evil.test` | `admin@target.com` | `mail.target.com` | fail | pass | relaxed 可能 pass | `adkim=r` 子域对齐 |
| `bounce@evil.test` | `admin@target.com` | 无 | fail | fail | p=none 仍投递 | 只靠收件方策略 |

```python
# email_alignment_oracle.py — 从 Authentication-Results 归一化判断
import re

def parse_auth_results(header):
    fields = {}
    for key in ["spf", "dkim", "dmarc"]:
        m = re.search(rf"\b{key}=(pass|fail|softfail|neutral|temperror|permerror)", header, re.I)
        fields[key] = m.group(1).lower() if m else "unknown"
    dkim_d = re.search(r"header\.d=([^;\s]+)", header, re.I)
    spf_smtp = re.search(r"smtp\.mailfrom=([^;\s]+)", header, re.I)
    fields["dkim_d"] = dkim_d.group(1).lower() if dkim_d else None
    fields["smtp_mailfrom"] = spf_smtp.group(1).lower() if spf_smtp else None
    return fields

def classify_delivery(auth_results, inbox_label):
    parsed = parse_auth_results(auth_results)
    return {
        "auth": parsed,
        "receiver_label": inbox_label,
        "dmarc_pass": parsed["dmarc"] == "pass",
        "interesting": parsed["dmarc"] != "pass" and inbox_label in {"inbox", "primary"},
    }
```
...
def comprehensive_email_auth_check(domain):
    ...
    results = {
        ...
        'dmarc': None,
        'findings': [],
        'overall_score': 0,  # 0=最易伪造, 10=最难伪造
    }
    ...
    results['overall_assessment'] = (
        'easy_spoof' if results['overall_score'] < 5
        else 'partial_alignment_gap' if results['overall_score'] < 8
...
- `dns_email_records.json`: SPF、DKIM selectors、DMARC、MX、TTL、resolver。
- `alignment_matrix.csv`: MAIL FROM、Header.From、DKIM d=、SPF/DKIM/DMARC pass/fail。
- `receiver_results.json`: Gmail/Outlook/本地 SMTP 的 Authentication-Results、投递位置、退信原因。
- `header_injection_attempts.json`: 参数名、CRLF 变体、响应状态、是否生成额外 header。
- 成功样本: DMARC fail 仍进 inbox、子域 relaxed alignment 通过、`l=` 允许 body 尾部被追加、header injection 生成额外字段。
- 失败样本: `p=reject` 且 strict alignment 生效、接收方退信、CRLF 被框架规范化、DKIM body hash mismatch。
