...
参数篡改必须盯最终结算点，而不是中间接口。前端显示金额、创建订单金额、支付网关金额、回调确认金额、发货金额可能来自不同字段。

```text
cart → quote → order → payment_intent → callback → fulfill/refund
       ↑        ↑       ↑                ↑          ↑
       价格     订单总额 网关金额          回调金额    发货/权益
```

| 阶段 | 观察字段 | 攻击动作 | 命中信号 |
|---|---|---|---|
| cart | `price`, `qty`, `sku` | 改数量/单位/sku | 购物车金额异常 |
| quote | `shipping`, `tax`, `discount` | 改运费/税费/优惠 | quote 接受 |
| order | `total`, `payable`, `currency` | 改最终订单金额 | 订单总额持久化 |
| payment | `amount`, `currency`, `gateway` | 网关金额错配 | 低价 payment intent |
| callback | `paid_amount`, `status` | 回调参数错配 | 订单变 paid |
| fulfill | `entitlement`, `license`, `download` | 跳过支付后发货 | 权益到账/Flag |
...
| 样本 | 判断 |
|---|---|
| cart 显示 0，但 order total 恢复原价 | 前端/quote 层可改，结算层重算 |
| order total 为 0，payment intent 为原价 | 网关层重算 |
| payment intent 低价，callback 后订单 paid | 完整命中 |
| coupon apply 成功，checkout 报不可用 | 优惠券只在 quote 层成功 |
| 多线程 10 次返回 200，但只有 1 个权益到账 | 接口响应竞态，业务落库原子 |
| 负数金额产生退款/余额增加 | 负值路径可继续 |

金额字段要同时记录“请求值、响应值、持久化值、网关值、最终权益”。如果四个值不一致，优先攻击最靠后的那个。
