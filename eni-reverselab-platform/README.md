...
1. Clone into a stable local directory, for example `<workspace>/eni-reverselab-platform`.
2. Windows: double-click `START_HERE.bat` or `START_HERE.cmd` for the first-run check. macOS/Linux: run `./START_HERE.sh`.
3. Claude Code: `cd <workspace>/eni-reverselab-platform` before starting the session.
4. Codex APP: open the existing `eni-reverselab-platform` folder directly.
